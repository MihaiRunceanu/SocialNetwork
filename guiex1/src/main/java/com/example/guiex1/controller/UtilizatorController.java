...
public class UtilizatorController implements Observer<Event> {
    ...
    private void initModel() {
        Iterable<Utilizator> messages = service.getAllUsers();
    ...
    public void handleLogInUtilizator(ActionEvent actionEvent) throws IOException {
        Utilizator user = (Utilizator) tableView.getSelectionModel().getSelectedItem();
        if (user != null) {
            FXMLLoader fxmlLoader = new FXMLLoader(UtilizatorController.class.getResource("../views/friendsview.fxml"));

            AnchorPane friendshipLayout = fxmlLoader.load();
            Stage secondaryStage = new Stage();
            secondaryStage.setScene(new Scene(friendshipLayout));

            PrietenieController friendsController = fxmlLoader.getController();
            friendsController.setPrietenieService(service, user);
